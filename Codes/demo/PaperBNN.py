...
def hessian(output, inputs, out=None, allow_unused=False, create_graph=False):
    ...
    n = sum(p.numel() for p in inputs)  # numel()函数：返回数组中元素的个数
    if out is None:
        out = output.new_zeros(n, n)

    ai = 0
    for i, inp in enumerate(inputs):  # 在同时需要index和value值的时候可以使用 enumerate
...
def main():
    ...
    subn = args.batch_train  # 100

    prior_sigma_0 = args.sigma0  # 0.0001

    prior_sigma_1 = args.sigma1  # 0.01

    lambda_n = args.lambdan  # 0.00001

    num_hidden = args.layer  # 3
    hidden_dim = args.unit  # [6,4,3]

    num_seed = args.num_run  # 1

    num_epoch = args.nepoch  # 500
    ...
    for my_seed in range(num_seed):
        ...
        for epoch in range(num_epoch):
            ...
            for iter in range(ntrain // subn):  # // 整除
            ...
            with torch.no_grad():
                ...
                # 按列返回最大元素，大于threshold的记为1，统计一共有多少个
        # 训练结束
        import pickle
        ...
        for epoch in range(fine_tune_epoch):
            ...
            with torch.no_grad():
                ...
                for i, para in enumerate(net.parameters()):
                    ...
                    para_gamma_path_fine_tune[i][epoch,] = (para.abs() > threshold).cpu().data.numpy()
